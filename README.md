...
FormPilot AI is a local-first Chrome extension that helps job seekers complete application forms faster with profile-based autofill, deterministic rules, and optional local AI support through Ollama.

Source and project updates: https://github.com/lovesidhuy
...
3. Click **Fill Page** to autofill using your profile, rules, and optional AI support.
...
- Default model: `phi3:mini` (configurable in Settings)
- Use the **Settings** tab to set the API base URL and model name, then click **Test** to verify connectivity.
...
| Ollama not responding | Ensure Ollama is running and `OLLAMA_ORIGINS` is set, then run the verify `curl` above |
...
Run the rule regression tests with:

```bash
node --test tests/*.test.js
```

This currently covers the Canada work authorization vs sponsorship routing logic in `rules.js`.
